...
device_quirk_id(){
    echo "LEGION-PIPEWIRE-CFG"
}

# Returning the name of the fix to display.
device_quirk_name(){
    echo "Lenovo Legion Go Pipewire Configuration"
}

# Do the install here.
device_quirk_install(){

    if [[ -d "${DQ_WORKING_PATH}/etc/pipewire" ]] && [[ ! -z $(diff -qr "${DQ_WORKING_PATH}/etc/pipewire" "${DQ_PATH}/scripts/lenovo/resources/LEGION-PIPEWIRE-CFG/pipewire") ]]; then
        return 2
    elif [[ -d "${DQ_WORKING_PATH}/etc/pipewire" ]] && [[ ! -e "${DQ_WORKING_PATH}/etc/pipewire/.LEGION-PIPEWIRE-CFG" ]]; then
        ...
        if [ $? -eq 0 ]; then
            cp -r "${DQ_PATH}/scripts/lenovo/resources/LEGION-PIPEWIRE-CFG/pipewire" "${DQ_WORKING_PATH}/etc/"
            touch "${DQ_WORKING_PATH}/etc/pipewire/.LEGION-PIPEWIRE-CFG"
            return 0
        else
...
device_quirk_removal(){
    ...
    if [[ -d "${DQ_WORKING_PATH}/etc/pipewire" ]] && [[ -z $(diff -qr "${DQ_WORKING_PATH}/etc/pipewire" "${DQ_PATH}/scripts/lenovo/resources/LEGION-PIPEWIRE-CFG/pipewire") ]]; then
        ...
        echo "Manual removal of the Legion Go pipewire configuration required..."
        return 1
    else
        return 2
    fi
