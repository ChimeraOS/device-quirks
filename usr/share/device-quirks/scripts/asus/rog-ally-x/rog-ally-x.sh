...
if [[ -d "${ALLY_X_WIREPLUMBER_DIR}" ]]; then
  echo "Installing wireplumber configs from ${ALLY_X_WIREPLUMBER_DIR} to ${WIREPLUMBER_DIR}"
  if [[ ! -d "${WIREPLUMBER_DIR}" ]]; then
    mkdir -p "${WIREPLUMBER_DIR}"
  fi

  # Copy the WirePlumber configs to /etc
  cp "${ALLY_X_WIREPLUMBER_DIR}"/* "${WIREPLUMBER_DIR}"
  echo "WirePlumber configurations successfully copied to ${WIREPLUMBER_DIR}"
else
  echo "WirePlumber configs not found at ${ALLY_X_WIREPLUMBER_DIR}"
  exit 1
fi
